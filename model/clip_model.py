...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        return self.resblocks(x), self.resblocks[-4](x).permute(1, 0, 2)
...
class CLIP(nn.Module):
    ...
    def decode_text(self, text, v_tokens, mlm_labels):
        with torch.no_grad():
            mlm_labels = 1. * (mlm_labels>0)
            mlm_labels = mlm_labels.unsqueeze(-1)
            #with torch.no_grad():
            x = self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]
            x = x * (1-mlm_labels) + v_tokens * mlm_labels
            #with torch.no_grad():        
            x = x + self.positional_embedding.type(self.dtype)
            x = x.permute(1, 0, 2)  # NLD -> LND
            x, x_m = self.transformer(x.half())
            x = x.permute(1, 0, 2)  # LND -> NLD
            x = self.ln_final(x).type(self.dtype)
    ...
    def decode_text1(self, text, v_tokens):
        #pdb.set_trace()
        with torch.no_grad():
            x = v_tokens + self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]
            #for i in range(v_tokens.shape[1]):
            #    x[:,st+i,:] = v_tokens[:,i,:]
            x = x + self.positional_embedding.type(self.dtype)
            x = x.permute(1, 0, 2)  # NLD -> LND
            x, x_m = self.transformer(x)
    ...
    def forward(self, image, text):
        image_features, image_m = self.encode_image(image)
        #with torch.no_grad():
        text_features, text_m = self.encode_text(text)
        ...
        return image_features, image_m, text_features, text_m
